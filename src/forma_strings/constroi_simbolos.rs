...
// caminhos comuns:
const CAMINHO_ALFABETO:&str = "../texto_desenho/simbolos/alfabeto/";
const CAMINHO_NUMEROS:&str = "../texto_desenho/simbolos/numeros/";
...
pub fn carrega_desenhos_numeros() -> HashMap<u8, Vec<Vec<char>> > {
  ...
  /* tabela contendo o número inteiro e uma estrutura de 
  * dados para sua representação gráfica de texto.*/
   ...
   * seus respectivos índices: */
   ...
   let pares:[(u8, &str); 10] = [(1,"um.txt"),   (2,"dois.txt"), 
...
pub fn carrega_caracteres_pontuacao() -> HashMap<char, Vec<Vec<char>>> {
   ...
   let caminho = Path::new("simbolos/outros");
   ...
   let equivalente = {
      [("abre_aspas_duplas",'\"'), ("abre_chaves",'{'), 
       ...
       ("seta_baixo",'\u{2193}'), ("seta_cima",'\u{2191}'), 
       ("seta_esquerda",'\u{2190}'), ("til", '~'), ("traco",'-'), 
       ("velha",'#'), ("virgula", ',')]
   };
...
mod tests {
   ...
   #[test]
   fn carrega_simbolos() {
      let outros = super::carrega_caracteres_pontuacao();
   ...
   fn matematica_simbolos() {
      let simbolos = super::carrega_caracteres_pontuacao();
